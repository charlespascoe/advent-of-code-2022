...
func findUnknown(node any, prev int) int {
	switch n := node.(type) {
	case Operation:
		var nextVal int
		var next any
		...
		if lit, ok := n.Left.(Literal); ok {
			nextVal = n.Op.FindUnknown(false, lit.Val, prev)
			next = n.Right
		} else if lit, ok := n.Right.(Literal); ok {
			nextVal = n.Op.FindUnknown(true, lit.Val, prev)
			next = n.Left
		} else {
			panic("ast not simplified")
		}

		return findUnknown(next, nextVal)
